...
import { fetchRevenue, RevenueSource } from '../../../services/adminFinanceService';
...
export default function AdminRevenue() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        setIsLoading(true);
        setError(null);
        const data = await fetchRevenue(selectedPeriod);
  ...
  const daysDivisor = selectedPeriod === 'year' ? 365 : 30;
  const avgDailyRevenue = totalRevenue / daysDivisor;
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="flex items-center justify-center h-64">
            <div className="flex items-center gap-3">
              ...
              <span className="text-gray-600">جاري تحميل الإيرادات...</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-200">
            ...
            <p className="text-3xl font-bold text-gray-900">{totalRevenue.toLocaleString()} ج.م</p>
            <p className="text-sm text-gray-600 mt-2">حسب الفترة المحددة</p>
          </div>
          <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-200">
            <p className="text-sm text-gray-600">متوسط الإيرادات اليومية</p>
            <p className="text-3xl font-bold text-gray-900">{Math.round(avgDailyRevenue).toLocaleString()} ج.م</p>
