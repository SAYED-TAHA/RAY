...
export default function AdminHeader() {
  ...
  const load = async () => {
    try {
      ...
      setSettings((prev) => ({
        ...prev,
        visible: data?.ui?.header?.visible ?? prev.visible,
        sticky: data?.ui?.header?.sticky ?? prev.sticky,
        transparent: data?.ui?.header?.transparent ?? prev.transparent,
        backgroundColor: data?.ui?.header?.backgroundColor ?? prev.backgroundColor,
        textColor: data?.ui?.header?.textColor ?? prev.textColor,
        logoSize: data?.ui?.header?.logoSize ?? prev.logoSize,
        menuPosition: data?.ui?.header?.menuPosition ?? prev.menuPosition,
        searchEnabled: data?.ui?.header?.searchEnabled ?? prev.searchEnabled,
        cartEnabled: data?.ui?.header?.cartEnabled ?? prev.cartEnabled,
        userMenuEnabled: data?.ui?.header?.userMenuEnabled ?? prev.userMenuEnabled,
        notificationsEnabled: data?.ui?.header?.notificationsEnabled ?? prev.notificationsEnabled,
        height: data?.ui?.header?.height ?? prev.height
      }));
    } catch (e) {
      console.error('Failed to load header settings:', e);
      setError('فشل تحميل إعدادات الهيدر');
    } finally {
      setLoading(false);
    }
  };
    ...
    try {
      ...
      await updateAdminSettings({
        ui: {
          header: { ...settings },
          visibility: {
            headerVisible: settings.visible
          }
        }
      } as any);
    } catch (e: any) {
      console.error('Failed to save header settings:', e);
      setError(e?.message || 'فشل حفظ إعدادات الهيدر');
    } finally {
      setSaving(false);
    }
  };
